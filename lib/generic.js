...
module.exports.parse = function(html,schema, callback) {
  ...
  var tagmatches={};
 var  custommap=schema || schemamobile.product;
  if (custommap) {
      ...
      if(custommap[customattrib].tag){
        $(custommap[customattrib].tag).filter(function (index, element) {
          ...
          if (Array.isArray(tagmatches[customattrib]) & valid) {
            ...
            tagmatches[customattrib].push({ node: element, text: tagtext, html: $(element).clone().wrap('<p/>').parent().html() });

          } else if (valid) {
            tagmatches[customattrib] = [{ node: element, text: tagtext, html: $(element).clone().wrap('<p/>').parent().html() }];
          }
        });
      }
    ...
    tagmatches[attribute].forEach(function (taginfo, index) {
      ...
      if (extractfunc) {
        ...
        extractfunc.pipeline.forEach(function (parsefunc, funindex) {
           ...
           if(extractfunc.pipeline.length==funindex+1){
             if(tagmatches[attribute].length>1){
                if(index==0){
                    product[attribute]=[parsedata];
                }else{
                  product[attribute].push(parsedata);
                }
