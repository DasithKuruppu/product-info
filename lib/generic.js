...
module.exports.parse = function(html, callback) {
  ...
  hierarchy.forEach(function(type, i){
    ...
    $(type).filter(function(index, element){  
      for (var productattribute in mapper){
          var pattern=mapper[productattribute].pattern;
          var tagtext=$(this).text().replace(/[\n\f\t]/g,'').trim(); // if there is a regex / pattern
          if(pattern && mapper[productattribute].tags.indexOf(type)!=-1 && (tagtext.match(new RegExp(pattern,'i')) || []).length > 0){
          ...
          if(!pattern && mapper[productattribute].tags.indexOf(type)!=-1){ //if no regex is defined
            if (Array.isArray(tagmatches[productattribute])) {
